...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    locale: "en_US",
    type: "article",
    images: [{ url: "https://res.cloudinary.com/dyr5ihrer/video/upload/q_80,f_jpg,w_1200,h_630,c_fill,so_0/v1771207837/gatesroof.com_Header_on1ccl.mov", width: 1200, height: 630, alt: "Best Roofing Company in ${city.name} Colorado - Gates Enterprises" }],
  },
};
...
const FAQS = [
  ...
  {
    q: "How do I choose a roofing company in ${city.name}?",
  ...
  {
    q: "Why do manufacturer certifications matter for ${city.name} roofing?",
    a: "Manufacturer certifications are earned, not purchased. They require contractors to demonstrate installation excellence, maintain customer satisfaction scores, and complete ongoing training. Certified contractors can offer extended manufacturer warranties that uncertified roofers cannot. For ${city.name} homeowners, this means better protection and longer lasting roofs."
  },
  {
    q: "Does Gates Enterprises offer free roof inspections in ${city.name}?",
    a: "Yes. Gates Enterprises LLC provides free, no obligation roof inspections for ${city.name} homeowners. Their inspectors assess your roof's condition honestly, document any damage with photos, and provide a clear recommendation. There is no pressure to commit to any work."
  }
];

const breadcrumbs = breadcrumbSchema([
  { name: "Home", url: "https://www.gatesroof.com" },
  { name: "Best Roofer ${city.name}", url: "https://www.gatesroof.com/best-roofer-${city.slug}" },
]);
...
function generateContentTsx(city) {
  const componentName = `BestRoofer${pascalCase(city.slug)}Content`;
  const nearbyLinks = city.nearby.map(l => `              { label: "${l.label}", href: "${l.href}" },`).join('\n');
    ...
    <div style={{ background: WHITE }}>
      ...
      <section style={{ padding: "160px 24px 80px", background: \`linear-gradient(170deg, \${NAVY} 0%, #0F2A42 100%)\`, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", position: "relative", zIndex: 1 }}>
          <Link href="/" style={{ fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.5)", textDecoration: "none" }}>Home &rarr; Best Roofer ${city.name}</Link>
      ...
      <section style={{ padding: "80px 24px", background: WHITE }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <h2 style={{ fontFamily: "var(--font-playfair), 'Playfair Display', Georgia, serif", fontSize: "clamp(28px, 4vw, 36px)", fontWeight: 800, color: NAVY, marginBottom: 16, lineHeight: 1.2 }}>
            What to Look for in a ${city.name} Roofing Company
          </h2>
          <p style={{ fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif", fontSize: 16, lineHeight: 1.85, color: TEXT_LIGHT, marginBottom: 32 }}>
            ${city.name} homeowners have many roofing contractors to choose from, and not all of them are equal. Before signing with any company, here are the factors that separate the best from the rest.
          </p>
...
for (const city of CITIES) {
  const dir = path.join(process.cwd(), 'app', `best-roofer-${city.slug}`);
  fs.mkdirSync(dir, { recursive: true });
  
  fs.writeFileSync(path.join(dir, 'page.tsx'), generatePageTsx(city));
  fs.writeFileSync(path.join(dir, 'content.tsx'), generateContentTsx(city));
  
  created++;
  console.log(`Created best-roofer-${city.slug}/ (page.tsx + content.tsx)`);
}

console.log(`\nDone! Created ${created} city pages (${created * 2} files total).`);
