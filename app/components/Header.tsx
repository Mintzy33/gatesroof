    ...
    <>
      <header style={{
        ...
        <div className="header-inner" style={{
          ...
          <nav className="desktop-nav" style={{ display: "flex", alignItems: "center", gap: 24, marginLeft: 32 }}>
            ...
            <Link href="/referral" style={{ fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif", fontSize: 15, fontWeight: 500, color: "rgba(255,255,255,0.85)", textDecoration: "none", transition: "color 0.3s" }}>
              Referral
            </Link>
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 999, background: WHITE, transform: menuOpen ? "translateX(0)" : "translateX(100%)", transition: "transform 0.35s cubic-bezier(0.16,1,0.3,1)", display: "flex", flexDirection: "column", overflowY: "auto" }}>
        ...
        <div style={{ padding: "32px 28px", flex: 1 }}>
          ...
          <Link href="/referral" onClick={() => setMenuOpen(false)} style={{ display: "block", padding: "20px 0", fontFamily: "var(--font-playfair), 'Playfair Display', Georgia, serif", fontSize: 32, fontWeight: 700, color: NAVY, textDecoration: "none", borderBottom: "1px solid rgba(13,33,55,0.06)", opacity: menuOpen ? 1 : 0, transform: menuOpen ? "translateY(0)" : "translateY(20px)", transition: `opacity 0.4s ${0.12 + links.length * 0.04}s, transform 0.4s ${0.12 + links.length * 0.04}s` }}>
            Referral
          </Link>
        </div>
